...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = velocity_mag * P_LOOKAHEAD_TIME;  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = ego_state.location.x + ego_state.velocity.x;  // <- Fix This
      goal.velocity.y = ego_state.location.y + ego_state.velocity.y;  // <- Fix This
      goal.velocity.z = ego_state.location.z + ego_state.velocity.z;  // <- Fix This
      ...
      goal.velocity.x = P_SPEED_LIMIT * std::cos(goal.rotation.yaw);  // <- Fix This
      goal.velocity.y = P_SPEED_LIMIT * std::sin(goal.rotation.yaw);  // <- Fix This
      goal.velocity.z = 0;
    }
    ...
    if (utils::magnitude(ego_state.velocity) <=
        _lookahead_distance_min) {  // -> Fix this
