...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations)
{
    ...
    icp.setRANSACOutlierRejectionThreshold (8);
...
int main()
{
	...
	while (!viewer->wasStopped())
	{
		...
		if (!new_scan)
		{
			...
			// transform = NDT(ndt, cloudFiltered, pose, 4); //TODO: change the number of iterations to positive number
			transform = ICP(mapCloud, cloudFiltered, pose,12);
